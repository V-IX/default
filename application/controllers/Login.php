	...
	public function index()
	{
		...
		if($_POST)
		{
			try
			{
				...
				if(empty($user))
					throw new Exception(fa('exclamation-triangle mr5').' Пользователь не найден!');
				
				if(!password_verify($password, $user['password']))
					throw new Exception(fa('exclamation-triangle mr5').' Пароль указан неверно!');
				
				if($user['delete'] == 1)
					throw new Exception(fa('exclamation-triangle mr5').' Ваш аккаунт был удален!');
				
				set_cookie(init_cookie('userid', $user['id']));
				set_cookie(init_cookie('hash', password_hash($user['id'].$user['hash'], PASSWORD_DEFAULT)));
				
				set_flash('site', action_result('success', fa('check mr5') . ' Добро пожаловать, <strong>' . $user['name'] . '</strong> в Ваш кабинет!'));
