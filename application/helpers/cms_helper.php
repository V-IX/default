...
if ( ! function_exists('admin_pagination'))
{
	function admin_pagination($base_url, $total, $per_page = 10, $uri_segment = 4)
	{
		$return = array(
            ...
            'num_tag_open'	=> '&nbsp;',
            'num_tag_close'	=> '',
            'cur_tag_open'	=> '<span class="num active">',
            'cur_tag_close'	=> '</span>',
            'anchor_class'  => 'num',
...
if ( ! function_exists('site_pagination'))
{
	function site_pagination($base_url, $total, $per_page = 10, $uri_segment = 2)
	{
		$return = array(
			...
			'first_link'	=> fa('arrow-left') . ' <span>В начало</span>',
			'last_link'		=> '<span>В конец</span> ' . fa('arrow-right'),
			'prev_link'		=> fa('arrow-left') . ' <span>Предыдущая</span>',
			'next_link'		=> '<span>Следующая</span> ' . fa('arrow-right'),
		);
