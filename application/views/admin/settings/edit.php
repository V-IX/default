<?=form_open_multipart('', ['class' => 'responsive-form', 'data-toggle' => 'entryform']);?>

<?=$this->admincreator
	->set_name('title')
	->set_label('Название сайта')
	->set_value($siteinfo['title'])
	->set_required()
	->create(); ?>

<?=$this->admincreator
	->set_name('descr')
	->set_label('Описание сайта')
	->set_value($siteinfo['descr'])
	->set_required()
	->create(); ?>

<?=$this->admincreator
	->set_name('owner')
	->set_label('Владелец сайта')
	->set_value($siteinfo['owner'])
	->create(); ?>

<?=$this->admincreator
	->set_name('details')
	->set_type('textarea')
	->set_label('Реквизиты')
	->set_value($siteinfo['details'])
	->create(); ?>

<?=$this->admincreator
	->set_name('copyright')
	->set_type('textarea')
	->set_label('Копирайт')
	->set_value($siteinfo['copyright'])
	->create(); ?>

<hr class="mt30 mb30" />

<div class="h3 bold mb20">Контакты</div>

<?=$this->admincreator
	->set_name('email')
	->set_label('E-mail')
	->set_value($siteinfo['email'])
	->set_required()
	->create(); ?>

<?=$this->admincreator
	->set_name('email_sender')
	->set_label('E-mail отправителя')
	->set_value($siteinfo['email_sender'])
	->set_required()
	->create(); ?>
	
<div class="row form-group">
	<div class="col-3 form-collabel">
		Телефон (основной): <span class="required">*</span>
	</div>
	<div class="col-9 form-colinput">
		<div class="row">
			<div class="col-6">
				<input type="text" class="form-input" name="phone" value="<?=set_value('phone', $siteinfo['phone']);?>" />
				<?=form_error('phone'); ?>
			</div>
			<div class="col-6">
				<input type="text" class="form-input" name="mask_phone" value="<?=set_value('mask_phone', htmlspecialchars_decode($siteinfo['mask_phone']));?>" readonly />
				<?=form_error('mask_phone'); ?>
			</div>
		</div>
	</div>
</div>
<div class="row form-group">
	<div class="col-3 form-collabel">
		Телефон (дополнительный):
	</div>
	<div class="col-9 form-colinput">
		<div class="row">
			<div class="col-6">
				<input type="text" class="form-input" name="phone_extra" value="<?=set_value('phone_extra', $siteinfo['phone_extra']);?>" />
				<?=form_error('phone_extra'); ?>
			</div>
			<div class="col-6">
				<input type="text" class="form-input" name="mask_phone_extra" value="<?=set_value('mask_phone_extra', htmlspecialchars_decode($siteinfo['mask_phone_extra']));?>" readonly />
				<?=form_error('mask_phone_extra'); ?>
			</div>
		</div>
	</div>
</div>
<div class="row form-group">
	<div class="col-3 form-collabel">
		Телефон (городской):
	</div>
	<div class="col-9 form-colinput">
		<div class="row">
			<div class="col-6">
				<input type="text" class="form-input" name="phone_city" value="<?=set_value('phone_city', $siteinfo['phone_city']);?>" />
				<?=form_error('phone_city'); ?>
			</div>
			<div class="col-6">
				<input type="text" class="form-input" name="mask_phone_city" value="<?=set_value('mask_phone_city', htmlspecialchars_decode($siteinfo['mask_phone_city']));?>" readonly />
				<?=form_error('mask_phone_city'); ?>
			</div>
		</div>
	</div>
</div>

<?=$this->admincreator
	->set_name('skype')
	->set_label('Skype')
	->set_value($siteinfo['skype'])
	->create(); ?>

<?=$this->admincreator
	->set_name('address')
	->set_type('textarea')
	->set_label('Адрес')
	->set_value($siteinfo['address'])
	->create(); ?>

<?=$this->admincreator
	->set_name('map')
	->set_type('textarea')
	->set_label('Код карты')
	->set_value(htmlspecialchars_decode($siteinfo['map']))
	->create(); ?>

<hr class="mt30 mb30" />

<div class="h3 bold mb20">Логотип компании</div>

<?=$this->admincreator
	->set_name('img')
	->set_type('file')
	->set_label('Изображение')
	->file_origin($siteinfo['img'], PATH_UPLOADS.'/'.$folder)
	->create(); ?>

<hr class="mt30 mb30" />

<div class="h3 bold mb20">SEO по умолчанию</div>

<?=$this->admincreator
	->set_name('meta_title')
	->set_label('Meta Title')
	->set_value($siteinfo['meta_title'])
	->set_required()
	->create(); ?>

<?=$this->admincreator
	->set_name('meta_description')
	->set_type('textarea')
	->set_label('Meta Description')
	->set_value($siteinfo['meta_description'])
	->create(); ?>

<?=$this->admincreator
	->set_name('img_alt')
	->set_label('Alt изображений')
	->set_value($siteinfo['img_alt'])
	->create(); ?>

<hr class="mt30 mb30" />

<div class="h3 bold mb20">Заглушка</div>

<?=$this->admincreator
	->set_name('enable')
	->set_type('checkbox')
	->set_label('Включить сайт')
	->set_value($siteinfo['enable'] == 1)
	->create(); ?>

<?=$this->admincreator
	->set_name('cap_title')
	->set_label('Заголовок для заглушки')
	->set_value($siteinfo['cap_title'])
	->create(); ?>

<?=$this->admincreator
	->set_name('cap_descr')
	->set_type('textarea')
	->set_label('Описание для заглушки')
	->set_value($siteinfo['cap_descr'])
	->create(); ?>

<hr class="mt30 mb30" />

<div class="h3 bold mb20">Продвинутое</div>

<?=$this->admincreator
	->set_name('color')
	->set_label('Цвет вкладки (HEX-code)')
	->label_info('Для мобильных устройств')
	->set_value($siteinfo['color'])
	->create(); ?>

<?=$this->admincreator
	->set_name('code_before')
	->set_type('textarea')
	->set_label('Код в шапке')
	->set_params(['rows' => 5])
	->set_value(htmlspecialchars_decode($siteinfo['code_before']))
	->create(); ?>

<?=$this->admincreator
	->set_name('code_after')
	->set_type('textarea')
	->set_label('Код в футере')
	->set_params(['rows' => 5])
	->set_value(htmlspecialchars_decode($siteinfo['code_after']))
	->create(); ?>

<div class="form-actions">
	<?=btn_save();?>
    <?=btn_save_exit();?>
    <?=btn_back($this->page);?>
</div>

<?=form_close();?>